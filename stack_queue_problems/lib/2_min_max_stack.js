...
class MinMaxStack {
    ...
    max(){
        return this.maximums[this.maximums.length - 1] || null
    }

    push(val) {
        const newNode = new Node(val);

        if (!this.minimums.length || newNode.value <= this.minimums[this.minimums.length - 1].value ) {
            this.minimums.push(newNode)
        }
        if (!this.maximums.length || newNode.value >= this.maximums[this.maximums.length - 1].value ) {
            this.maximums.push(newNode)
        }
    ...
    pop() {       
        ...
        if (this.top.value === this.minimums[this.minimums.length - 1].value){
            this.minimums.pop()
        }
        if (this.top.value === this.maximums[this.maximums.length - 1].value){
            this.maximums.pop()
        }
